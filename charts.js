...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var yticks = sampleValues.slice(0,10);
...
//   d3.json("samples.json").then((data) => {
  ...
  // 1. Create a variable that filters the metadata array for the object with the desired sample number.
	var resultArrayMeta = data.metadata.filter(sampleObj => sampleObj.id == sample);

  // 2. Create a variable that holds the first sample in the metadata array.
  var resultMeta = resultArray[0];

  // 3. Create a variable that holds the washing frequency.
  var washFrequency = resultArrayMeta.map((meta) => meta.wfreq)
  // 4. Create the trace for the gauge chart.
  var gaugeData = washFrequency.map(wfreq => {
    return {
      gauge: {
          axis: {
              range: [0, 10]
